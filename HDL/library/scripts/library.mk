...
XILINX_DEPS_ALL := $(or $(XILINX_DEPS), $(XILINX_DEPS_HLS))

ifneq ($(XILINX_DEPS_ALL),)

ifeq ($(XILINX_DEPS_HLS),)
...
ifneq ($(XILINX_DEPS_HLS),)
component.xml: $(XILINX_DEPS_HLS)
	-rm -rf $(CLEAN_TARGET)
	$(call build, \
		$(VIVADO_HLS) $(LIBRARY_NAME)_hls_ip.tcl -l $(LIBRARY_NAME)_hls_ip.log, \
		$(LIBRARY_NAME)_hls_ip.log, \
		$(HL)$(LIBRARY_NAME)$(NC) hls library)
	-rm -rf $(LIBRARY_NAME)/solution1/impl/ip/*.zip
	$(call build, \
		$(VIVADO) $(LIBRARY_NAME)_ip.tcl, \
		$(LIBRARY_NAME)_ip.log, \
		$(HL)$(LIBRARY_NAME)$(NC) repackage library)
	-mv $(LIBRARY_NAME)/solution1/impl/ip/* ./
	-rm -rf $(LIBRARY_NAME)
endif
